...
function PreviousProblems() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post("http://localhost:8000/Corporateuser/previous-reports", { Email: email });
  ...
  const handleFollowUp = async (reportId) => {
    try {
      const response = await axios.put(`http://localhost:8000/Corporateuser/follow-up/${reportId}`);
